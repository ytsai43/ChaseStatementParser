...
def convertStatement(fileName, inputPath = ".", outputPath = ".\csv"):
    ...
    DATE = rawData.iloc[idxTranDate]["rawData"].apply(lambda x: x[-5:] if len(x)> 5  else x)
    ...
    Export to CSV
    """
    finalTable.to_csv("%s\%s.csv" %(outputPath, fileName[:-4]),header=True,index=False,encoding='utf_8_sig')
    pdf_file.close()
